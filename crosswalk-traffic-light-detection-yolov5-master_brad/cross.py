...
# 횡단보도,신호등 모델
MODEL_PATH = './crosswalk-traffic-light-detection-yolov5-master_brad/runs/train/exp4/weights/best.pt'
...
net = cv2.dnn.readNetFromDarknet('./crosswalk-traffic-light-detection-yolov5-master_brad/models/yolov4-ANPR.cfg', './crosswalk-traffic-light-detection-yolov5-master_brad/models/yolov4-ANPR.weights')

# 동영상 로드
cap = cv2.VideoCapture('./crosswalk-traffic-light-detection-yolov5-master_brad/data/full3.mp4')

fourcc = cv2.VideoWriter_fourcc('m', 'p', '4', 'v')
out = cv2.VideoWriter('./crosswalk-traffic-light-detection-yolov5-master_brad/data/output.mp4', fourcc, cap.get(cv2.CAP_PROP_FPS), (int(cap.get(cv2.CAP_PROP_FRAME_WIDTH)), int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))))
